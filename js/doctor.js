...
Doctor = function(firstName, lastName, title, gender, bio, currentPractice, phone, image){
...
Doctor.prototype.getDoctors = function(medicalIssue, displayDoctors) {
  ...
  $.get('https://api.betterdoctor.com/2016-03-01/doctors?query='+ medicalIssue+'&location=45.5231%2C-122.6765%2C%205&user_location=45.5231%2C-122.6765&skip=0&limit=20&user_key=' + apiKey).then(function(response) {
    ...
    for (var i = 0; i < response.data.length; i++) {
      var newDoctor = new Doctor(firstName=response.data[i].profile.first_name, lastName=response.data[i].profile.last_name, title=response.data[i].profile.title, gender=response.data[i].profile.gender, bio=response.data[i].profile.bio, practice=response.data[i].practices[0].name, phone=response.data[i].practices[0].phones[0].number, image=response.data[i].profile.image_url);
