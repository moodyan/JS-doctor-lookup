...
Doctor.prototype.getDoctorInfo = function(npi, displayDoctorInfo) {
  $.get('https://api.betterdoctor.com/2016-03-01/doctors/npi/'+npi+'?user_key=' + apiKey).then(function(response) {
    console.log(response);
    displayDoctorInfo(firstName=response.data.profile.first_name, lastName=response.data.profile.last_name, title=response.data.profile.title, gender=response.data.profile.gender, bio=response.data.profile.bio, practice=response.data.practices[0].name, phone=response.data.practices[0].phones[0].number, image=response.data.profile.image_url, npi=response.data.npi);
  });
};
